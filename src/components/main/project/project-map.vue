<template>
  <div>
    <div id="mapDiv" style="position:absolute;width:100%; height:86vh"></div>
...
<script>
  export default {
    components: {
      'right-sidebar': require('../utility/right-sidebar.vue')
    },
    props: {
      info: {
        type: Object,
        ...
        window.appContext.http.getProjectPage_R(accountId).then((response) => {
          ...
          if (transition.from.path === '/login') {
            if (projects.length === 1) {
              window.sessionUtility.setObj(window.sessionKeys.PROJECT, projects[0]);
              this.$route.router.go({path: '/project/situationInfo/realTime'});
            } else {
              transition.next();
            }
          } else {
...
//        $.AdminLTE.controlSidebar.close();
    ...
    mounted (){
      this.loadData(window.sessionUtility.getObj(window.sessionKeys.ACCOUNT).id);
      this.getProjectIdsExistMap();
    },
    methods: {
      loadData(accountId){
        window.appContext.http.getProjectPage_R(accountId).then((response) => {
          this.lnglats = response.body;
          if (this.lnglats.length > 0) {
            if (this.lnglats.length === 1) {
//                  this.$route.router.go({path: '/project/situationInfo/realTime'});
              window.sessionUtility.setObj(window.sessionKeys.PROJECT, this.lnglats[0]);
//                  return;
            }
            //预留功能。为给各项目添加状态字段
...
//                          this.info=lonlat;
      ...
      getProjectIdsExistMap () {
        window.appContext.http.getProjectIdsExistMap().then((response) => {
          window.sessionUtility.setObj(window.sessionKeys.PROJECTIDSEXISTMAP, response.body);
//                    console.log(response.body);
//                    console.log(window.sessionUtility.get(window.sessionKeys.PROJECTIDSEXISTMAP));
        }, (response) => {
