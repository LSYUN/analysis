    ...
    data () {
      return {
        ...
        operations: {
          add: {
            ...
            confirm: function (info) {
              window.appContext.http.monitorItemAdd(info).then((response) => {
                if (response.body.MESSAGE === 'success') {
                  this.$route.router.go({path: '/project/edit/itemList'});
                  toastr.info('添加成功!');
                } else {
          ...
          modify: {
            ...
            path: 'modify',
            confirm: function (info) {
              window.appContext.http.monitorItemModify(info).then((response) => {
                if (response.body.MESSAGE === 'success') {
                  this.$route.router.go({path: '/project/edit/itemList'});
                  toastr.info('修改成功!');
                } else {
        ...
        monitorTypes: {
          ...
          ajaxUrl: function () {
            return window.appContext.urls.getMonitorTypePage();
          }.bind(this)
        }
      };
    },
