...
//          retrieve: true,
          ...
          deferRender: true,
          ajax: {
            url: window.appContext.urls.getRelPointOfItems(itemId),
            type: 'GET',
          ...
          buttons: [
            {
              ...
              window.appContext.http.relItemRemovePoint(JSON.stringify(pointIds), {
                params: {
...
//          serverSide: true,
          ajax: {
            url: window.appContext.urls.getUnRelPointOfItems(itemId),
            type: 'GET',
            contentType: 'application/json',
            dataType: 'json',
            dataFilter: function (data) {
          ...
          buttons:[{text: '添加',className:'',action:function(e,dt,node,config){
            ...
            window.appContext.http.relItemAddPoint(list).then((response) => {
...
//          serverSide: true,
          ajax: {
            url: window.appContext.urls.getRelItemPointGroup(itemId),
            type: 'GET',
            contentType: 'application/json',
            dataType: 'json',
            dataFilter: function (data) {
...
//          serverSide: true,
          ajax: {
            url: window.appContext.urls.getNotRelItemPointGroup(itemId),
            type: 'GET',
            contentType: 'application/json',
            dataType: 'json',
            dataFilter: function (data) {
      ...
      updateTable(monitorItemId){
        this.relItemPointTable.ajax.url(window.appContext.urls.getRelPointOfItems(monitorItemId)).load();
        this.unRelItemPointTable.ajax.url(window.appContext.urls.getUnRelPointOfItems(monitorItemId)).load();
        this.relItemPointGroupTable.ajax.url(window.appContext.urls.getRelItemPointGroup(monitorItemId)).load();
        this.unRelItemPointGroupTable.ajax.url(window.appContext.urls.getNotRelItemPointGroup(monitorItemId)).load();
      },
      itemAddPoint(){
...
//          console.log(pointGroupIds);
        ...
        window.appContext.http.relItemPointGroupAdd(list).then((response) => {
...
//                    toastr.info('成功添加测点!');
      ...
      itemRemovePointGroup(){
        ...
        window.appContext.http.relItemPointGroupDelete(JSON.stringify(pointGroupIds), {
          params: {
            itemId: this.monitorItemId,
          }
        }).then((response) => {
