<template>
  <div class="box box-solid">
    ...
    <div class="box-body">
      <table id="monitorItem1" cellpadding="0" cellspacing="0" border="0" width="100%"
             class="display muchColumn hover">
        <thead>
        <tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      initTable () {
        let that = this;
        this.table = $('#monitorItem1').DataTable({
          retrieve: true,
          scrollX: true,
          showCheckbox: true,
          processing: true,
//          serverSide: true,
          ...
          deferRender: true,
          ajax: {
            url: window.appContext.urls.getMonitorItemPage_U(this.projectId),
            type: 'GET',
...
//              console.log(json);
      ...
      addInfo () {
        ...
        this.$route.router.go({path: '/project/edit/itemInfo/add'});
      },
      projectSetting(){
        const component = this;
        $('#monitorItem1 tbody').on('click', 'td:nth-child(3)', function (p) {
//          component.info.itemInfo = component.table.row(this).data();
          component.info.itemObj = component.table.row(this).data();
          window.sessionUtility.setObj(window.sessionKeys.ITEMOBJ,component.info.itemObj);
          component.$route.router.go({path: '/project/edit/projectSet'});
        });
      },
      modifyInfo(){
        const component = this;
        $('#monitorItem1 tbody').on('click', 'td:nth-child(4)', function (p) {
          ...
          component.$route.router.go({path: '/project/edit/itemInfo/modify'});
        });
      },
      deleteInfo(){
        const component = this;
        $('#monitorItem1 tbody').on('click', 'td:nth-child(5)', function (p) {
          ...
          layer.open({
            ...
            yes: function (index, layero) {
              window.appContext.http.monitorItemDelete(JSON.stringify(oneId)).then((response) => {
