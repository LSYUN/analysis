  ...
  export default{
    ...
    mounted(){
      ...
      if (!this.info || !this.info.itemObj || !this.info.itemObj.id) {
        this.info.itemObj = window.session.getObj(window.sessionKeys.ITEMOBJ);
      }
...
//          serverSide: true,
          ...
          ajax: {
            url: window.mainConfig.url.selectConfigNamesU(projectId, monitorItemId),
            type: 'GET',
      ...
      initEdit(){
        ...
        $("#example tbody").on("click", ".edit-btn", function () {
          window.mainConfig.http.selectConfigNamesR(that.projectId, that.info.itemObj.id).then((response) => {
...
//        console.log(showConf.json);
        window.mainConfig.http.getMonitorItemShowConf(showConf).then((response) => {
          toastr.success('保存成功!');
        }, (response) => {
...
//          window.mainConfig.http.selectConfigNames(showConf).then((response) => {
