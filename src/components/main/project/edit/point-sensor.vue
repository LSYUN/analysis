    ...
    data () {
      return {
        ...
        monitorItems: {
          ...
          evtSelected: function (evt, data) {
            if (data && data.length > 0) {
              ...
              window.appContext.http.getFirstPointOfItem(this.info.itemObj.id).then((response) => {
        ...
        monitorPoints: {
          ...
          ajaxUrl: function () {
//            return window.appContext.urls.getMonitorPointPage(this.projectId);
    ...
    methods: {
      ...
      getDeviceType(monitorPointId){
        this.deviceOptions = [];
        window.appContext.http.getRelSensorByPoint(monitorPointId).then((response) => {
...
//                    toastr.error(response.body);
      ...
      getDeviceData(pointId, sensorId){
        window.appContext.http.getItemDevicePara(pointId, sensorId).then((response) => {
          let data = response.body;
          console.log(data);
          if (data.length > 0) {
