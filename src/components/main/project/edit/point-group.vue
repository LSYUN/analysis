...
//            return window.appContext.urls.getMonitorPointPage(this.projectId);
    ...
    methods: {
      ...
      getFirstGroupOfItem(){
        window.appContext.http.getFirstPointGroupOfItem(this.info.itemObj.id).then((response) => {
          let firstGroupOption = response.body[0];
...
//          this.monitorPointGroups.initData.push({id: pointGroup.id, text: pointGroup.name, obj: pointGroup})
      ...
      getDeviceData(groupId){
        window.appContext.http.getPointGroupParams(groupId).then((response) => {
