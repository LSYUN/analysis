...
//              }
          ...
          ajaxUrl: function () {
            return window.appContext.urls.getRelPointOfItems(this.info.itemObj2.id);
          }.bind(this)
        },
        attrValueOption1: {
          initData: [],
    ...
    created: function () {
      this.$on('updateItem', function (data) {
        if (data) {
          ...
          this.info.itemObj = {};
          this.$refs.itemSelector1.$emit('clear', true);
        }
      });
      this.$on('updateItem2', function (data) {
        if (data) {
          ...
          this.info.itemObj = {};
          this.$refs.itemSelector2.$emit('clear', true);
        }
      });
      this.$on('updatePoint', function (data) {
        if (data && data.length > 0) {  //这里的data就是父组件中的this.info,pointObj
          ...
          this.$refs.pointSelector1.$emit('clear', true);
      ...
      this.$on('updatePoint2', function (data, autoReq) {
        if (data) {
          this.$refs.pointSelector2.$emit('clear', true);
      ...
      getChartData(itemObj1, itemObj2, pointObj1, pointObj2, Url1, Url2, attrValue1, attrValue2, attrText1, attrText2, attrUnit1, attrUnit2){
        if (itemObj1) {
          if (Url1 && Url2 && attrValue1 && attrValue2) {
            ...
            window.appContext.http.getMonitorItemData_R(Url1, 2, itemObj1.id, this.projectId, pointName1,
              ...
              dataList1 = response.body.dataList;
              window.appContext.http.getMonitorItemData_R(Url2, 2, itemObj2.id, this.projectId, pointName2,
                this.info.startEndDate.startDate.dateL, this.info.startEndDate.endDate.dateL, {
                  syncNo: 1, pageIndex: 1, pageSize: 1000
                }, itemType2).then((response) => {
                dataList2 = response.body.dataList;
