...
<script>
  export default {
//    components: {
//      'analysis': require('../utility/analyse-chart.vue'),//测斜孔 内部位移 2
//    },
    props: {
      info: {
        type: Object,
...
//        variety: 'clinometerHoleData',
    ...
    created: function () {
      this.$on('filterChart', function (e) {
        ...
        this.filterChart(itemOption.Url, itemOption.attrValue, e.projectId, itemOption.pointNames, startDate, endDate, e.attrText, e.attrUnit);
      });
    ...
    mounted () {
      ...
      this.chart.showLoading("<img src='../static/image/spinner_B.gif'>");
      let e = this.info;
      this.filterChart(e.itemOption.Url, e.itemOption.attrValue, this.projectId, e.pointNameList,
        e.startEndDate.startDate.dateL, e.startEndDate.endDate.dateL, e.itemOption.attrText, e.itemOption.attrUnit);
    },
    methods: {
      filterChart(variety, attrY, projectId, pointNames, startTime, endTime, attrText, attrUnit, pageIndex){
        window.mainConfig.http.getDataByPointAndTime_R(variety, projectId, pointNames, startTime, endTime, {
      ...
      createChart() {
        ...
        this.getChartOptions(this.info.itemOption.attrText, this.info.itemOption.attrUnit, this.info.itemOption.mark).title.text = '测斜孔监测';
...
//              });
      ...
      getChartOptions(text, unit, mark){
        return {
          ...
          title: {text: '测斜孔监测', style: {color: '#7cb5ec', fontSize: '1.8em', fill: '#7cb5ec'}},
          subtitle: {
