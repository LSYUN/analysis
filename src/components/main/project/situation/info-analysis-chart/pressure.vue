...
<script>
  export default {
//    components: {
//      'analysis': require('../utility/analyse-chart.vue'),//压强 17
//    },
    props: {
      info: {
        type: Object,
    ...
    created: function () {
      ...
      this.$on('updateChart', function (obj) {
        ...
        if (this.chart.series && this.chart.series.length > 0 && obj) {
          for (let i = 0, len = this.chart.series.length i < len;
          i++
        ))
      {
...
//        e.startEndDate.startDate.dateL, e.startEndDate.endDate.dateL);
    },
    methods: {
        ...
        window.mainConfig.http.getMonitorItemData_R(variety, urlType, monitorItemId, projectId, pointNames, startTime, endTime, {
          ...
          if (response.data && response.data.dataList) {
            ...
            if (this.dataList && this.dataList.length > 0) {
              ...
              this.createChart(this.seriesOptions);
            } else {
...
//              this.chart.hideLoading();
        ...
        ))
  {
      ...
      createChart() {
        ...
        this.getChartOptions(this.info.itemOption.attrText, this.info.itemOption.attrUnit,this.info.itemOption.mark).title.text = '压强监测';
...
//              });
      ...
      getChartOptions(text, unit,mark){
        return {
          ...
          title: {text: '压强监测', style: {color: '#7cb5ec', fontSize: '1.8em', fill: '#7cb5ec'}},
          subtitle:{
