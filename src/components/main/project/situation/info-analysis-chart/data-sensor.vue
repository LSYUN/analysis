...
<script>
  export default {
//    components: {
//      'analysis': require('../utility/analyse-chart.vue'),//原始数据 50
//    },
    props: {
      info: {
        type: Object,
      ...
      getSeriesOptions(data, attrY){
        ...
        if (data) {
          for (let i = 0; i < data.length; i++) {
            ...
            if (!map[ai.pointName]) {
              dest.push({
                name: ai.pointName + '(' + ( ai.unit ? ai.unit : '无单位') + ')',
              ...
              for (let j = 0; j < dest.length; j++) {
                let dj = dest[j];
                let name = dj.name.split(/(\()(?!.*\1)/)[0];
                if (name === ai.pointName) {
...
//              });
      ...
      getChartOptions(text, unit, mark){
        return {
          ...
          title: {text: '原始数据监测', style: {color: '#7cb5ec', fontSize: '1.8em', fill: '#7cb5ec'}},
          subtitle: {
