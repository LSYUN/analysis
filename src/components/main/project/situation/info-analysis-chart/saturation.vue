...
<script>
  export default {
    ...
    data () {
      return {
//        variety: 'saturationLineData',
        chart: {},
//        setPointName:this.info.pointNameList,            //用以判断查询类型的时候设置当前查询的点名
        setPointName: this.info.itemOption.pointNames,
        dataList: [],
        isRender: false,
        seriesOptions: {},
        pageIndex: 1,
    ...
    created: function () {
      this.$on('filterChart', function (e) {
        ...
        this.filterChart(e.Url, e.mark, e.monitorItemId, e.projectId, e.pointNames,
    ...
    methods: {
      filterChart(variety, urlType, monitorItemId, projectId, pointNames, startTime, endTime, attrY, attrText, attrUnit, pageIndex){
        window.mainConfig.http.getMonitorItemData_R(variety, urlType, monitorItemId, projectId, pointNames, startTime, endTime, {
          ...
          if (response.data && response.data.dataList && response.data.dataList.length > 0) { // 判断返回内容是否有数据
            ...
            this.createChart('浸润线监测', this.seriesOptions, attrText, attrUnit, urlType);
          } else {
      ...
      createChart(titleText, seriesOptions, attrText, attrUnit, mark) {
...
//              });
      ...
      getChartOptions(text, unit, mark){
        return {
          ...
          title: {text: '浸润线监测', style: {color: '#7cb5ec', fontSize: '1.8em', fill: '#7cb5ec'}},
          subtitle: {
