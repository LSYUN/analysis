...
<script>
  export default {
//    components: {
//      'analysis': require('../utility/analyse-chart.vue'),//土壤水分 14
//    },
    props: {
      info: {
        type: Object,
    ...
    created: function () {
      this.$on('filterChart', function (e) {
        ...
        this.filterByPointAndTime(e.Url,e.attrValue,e.projectId,e.pointNames, e.startDate, e.endDate, e.attrText, e.attrUnit);
      });
    ...
    mounted () {
      ...
      this.chart.showLoading("<img src='../static/image/spinner_B.gif'>");
      let e = this.info;
      this.filterByPointAndTime(e.itemOption.Url, e.itemOption.attrValue, this.projectId, e.pointNameList,
        e.startEndDate.startDate.dateL, e.startEndDate.endDate.dateL, e.itemOption.attrText, e.itemOption.attrUnit);
    },
    methods: {
      filterByPointAndTime(variety, attrY, projectId, pointNames, startTime, endTime,attrText, attrUnit, pageIndex){
      ...
      createChart() {
        ...
        this.getChartOptions(this.info.itemOption.attrText, this.info.itemOption.attrUnit,this.info.itemOption.mark).title.text = '土壤水分监测';
...
//              });
      ...
      getChartOptions(text, unit,mark){
        return {
          ...
          title: {text: '土壤水分监测', style: {color: '#7cb5ec', fontSize: '1.8em', fill: '#7cb5ec'}},
          subtitle:{
