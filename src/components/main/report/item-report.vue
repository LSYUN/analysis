      ...
      <div class="box-body">
        <div class="selection">
          <div class="row">
            ...
            <div class="col-sm-12 col-md-2 div-grid btn-container">
          ...
          <div class="con-box">
            <section class="content">
              ...
              <div class="reportMain">
                <table id="dataTable" border="0" class="display muchColumn table pointer" width="100%"></table>
                <div id="chart"></div>
                <div class="tail">
...
<script>
  ...
  import ReportEnum from '../../manager/enum/report-enum';
...
//      this.initProjectSelect();
    ...
    methods: {
      ...
      lsyTest(){
        window.appContext.http.lsyTest().then((response) => {
      ...
      /*c初始化下拉框*/
      initProjectSelect() {
        window.appContext.http.getProjectPage_R(this.accountId).then((response) => {
      ...
      prepareCondition(itemObj){
        ...
        if (this.monitorTypeId !== 20) {  // && this.monitorTypeId !== 7 排除全站仪 与 GPS, 导出数据与历史数据不同
          window.appContext.http.selectConfigNamesR(this.projectId, this.monitorItemId).then((response) => {
...
//          this.reportOption = ReportEnum.getReportConfig(this.monitorTypeId, this.attrValue);
      ...
      getQueryData(){
        ...
        window.appContext.http.getAlarmParamList(this.monitorItemId, '', 0).then((response) => {  //获取设置的限差数据
        ...
        function getReportData(controlValueAcc, controlValueRate, alarmClass, type) {
          window.appContext.http.getReportDataList(self.projectId, self.monitorItemId, self.monitorTypeId, startTime, endTime,
            controlValueAcc, controlValueRate, alarmClass, type,
            {syncNo: 1, pageIndex: 1, pageSize: 100000}).then((response) => {
            try {
              self.addOrModifySheet(response.body);
            } catch (e) {
              console.error(e)
            }
      ...
      exportExcel(){
        ...
        let options = {
          url: window.appContext.urls.getReportExport(), //请求的url
          data: {//要发送的数据
