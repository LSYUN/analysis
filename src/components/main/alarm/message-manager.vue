      ...
      return {
        ...
        projectOptions: {
          ...
          evtSelected: function (event, data) {
            ...
            this.table.ajax.url(window.appContext.urls.getAlarmMessage(this.projectId)).load();
            this.loading = true;
          }.bind(this),
          ajaxUrl: function () {
...
//          select: {style: 'single', selector: 'td:nth-child(20)'},
          ajax: {
            url: window.appContext.urls.getAlarmMessage(this.projectId),
            type: 'GET',
...
//                console.log(row);
      ...
      modify(info, callback){
        window.appContext.http.AlarmMessageModify(info).then((response) => {
          callback();
          toastr.success('修改成功');
        }), (response) => {
          console.log(response);
